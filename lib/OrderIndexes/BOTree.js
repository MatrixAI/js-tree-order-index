...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  _insertIntoLeaf ({
    ...
    if (leaf.space > 1) {
      ...
      if (leafRight.nextId != null) {
        console.log('UPDATING LEAF RIGHT ORIG 1, LEAFRIGHT ID, LEAFRIGHTNEXTID', leafRight.id, leafRight.nextId);
        ...
        console.log('UPDATED ID', leafRightOrig.prevId);
        console.log(leafRightOrig);
        treeTableT.set(leafRightOrig.id, leafRightOrig);
        console.log('UPDATED TREETABLE', treeTableT.get(1));
      }
  ...
  _insertIntoLeafPair({
    ...
    if (leaf.space > 2) {
      ...
      // to point its `prevId` to the new leafRight
      if (leafRight.nextId != null) {
        console.log('UPDATING LEAF RIGHT ORIG 2');
        let leafRightOrig = treeTableT.get(leafRight.nextId);
        if (!snapshot.has(leafRightOrig)) {
          leafRightOrig = leafRightOrig.clone();
          snapshot.add(leafRightOrig);
