...
function setupBlockConstructors (blockSize: number, nodeTable: NodeTableI) {

  // we fix a few things here
  // one is blockSize is always the same
  // this is just a way of creating hidden parameters
  // by hiding it in the closure
  // it cannot be seen
  // but sometimes you want public properties but that are immutable
  // so they are private properties
  ...
  class Leaf extends Block {
    ...
    _next: ?Leaf;
    _entries: Array<{ key: GapKey, entry: OrderEntry}>;
    ...
    relabelGapKeys () {
      for (let i = 0; i < this._lastEntryIndex; ++i) {
        const gapKeyNew = gapSize * (i + 1);
        const gapKeyEntry = this._entries[i];
        gapKeyEntry.key = gapKeyNew;
        const node = nodeTable.lookupById(gapKeyEntry.entry.id);
        if (node) {
          if (gapKeyEntry.status) {
            node.opening.updateGapKey(gapKeyNew);
          } else {
            node.closing.updateGapKey(gapKeyNew);
          }
        } else {
