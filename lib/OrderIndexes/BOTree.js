...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  _insertIntoLeafPair({
    ...
    // unlike insertIntoLeaf
    // this one does insert into both
    // and ensures that it succeeds
    // we may split and turn it into left and right block
    // but the gap key must then be important
    // so what does it mean to actually insert into leaf then?
    // if you have 2 leafs
    // then returning a single leaf makes no sense
    // maybe we can return leaf1 and leaf2
    // to represent the 2 leafs
    // but if there's no split, then you only get 1 leaf
    // then leaf1 and leaf2 are the same
  ...
  insertRoot (
    ...
    const [[
      ...
      if (this._leftId === this._rightId === this._rootId) {
        ...
        const {
          rootBlock: rootBlockNew,
          leftBlock: leftBlockNew,
          rightBlock: rightBlockNew,
          leaf1: leafOpen,
          gapKey1: gapKeyOpen,
          leaf2: leafClose,
          gapKey2: gapKeyClose
        ...
        cloneConfig.rootId = rootBlockNew.id;
        cloneConfig.leftId = leftBlockNew.id;
        cloneConfig.rightId = rightBlockNew.id;
      } else {
        // not the same
        // we have to insert into 2 locations
        ...
        // we may have a new root block
        const {
          rootBlock: blockRoot2,
          leftBlock: blockLeft2,
          rightBlock: blockRight2,
          leaf: leafOpen,
          gapKey: gapKeyOpen
        ...
        const {
          rootBlock: blockRoot3,
          leftBlock: blockLeft3,
          rightBlock: blockRight3,
          leaf: leafClose,
          gapKey: gapKeyClose
        } = this._insertIntoLeaf({
          nodeTableT: nodeTableT,
        ...
        cloneConfig.rootId = blockRoot3.id;
        cloneConfig.leftId = blockLeft3.id;
        cloneConfig.rightId = blockRight3.id;
      }

      const nodeInserted = nodeTableT.insertNode(
        level,
        openLink,
        closeLink,
        data_,
