...
class BOTree implements OrderIndexI<GapLink<Leaf>> {
  ...
  nextOpenCursor (cursor: Cursor): ?Cursor {
    const nextCursor = this.nextCursor(cursor);
    if (!nextCursor) return null;
    if (this.getEntry(nextCursor).status) {
      return nextCursor;
    } else {
      return this.nextOpenCursor(nextCursor);
    }
  }

  nextCloseCursor (cursor: Cursor): ?Cursor {
    const nextCursor = this.nextCursor(cursor);
    if (!this.getEntry(nextCursor).status) {
      return nextCursor;
    } else {
