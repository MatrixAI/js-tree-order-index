...
class Leaf extends Block<{key: number, entry: OrderEntry}> {
  ...
  // that's all
  insertEntry (position: number, orderEntry: OrderEntry): GapLink {
...
class BOTree implements OrderIndexI<GapLink<Leaf | Node>> {
  ...
  findEntry (link: GapLink<Leaf | Node>): OrderEntry {
    // do something with the link
  }

  // insertion into a BOTree returns back a GapLink to that entry
  insertEntry (entry: OrderEntry): GapLink<Leaf | Node> {
