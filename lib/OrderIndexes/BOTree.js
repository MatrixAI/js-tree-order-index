...
class BOTree implements OrderIndexI<GapLink<Leaf>, Cursor> {
  ...
  insertEntryPairChild (
    ...
    if (position > 0) {
      ...
      // iterate according to the parentLinkOpen

      // use parentLinkOpen and acquire the cursor
      const cursor = this.findCursor(parentLinkOpen);
      if (cursor) {

        // for each cursor, acquire the next cursor that is open
        // as in find the closing one
