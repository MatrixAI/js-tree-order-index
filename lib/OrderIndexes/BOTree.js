...
class BOTree implements OrderIndexI<GapLink<Leaf>, Cursor> {
  ...
  nextCursor (cursor: Cursor): ?Cursor {
    ...
    if (leaf.getChildByIndex(nextIndex)) {
      ...
      const nextLeaf = leaf.getLeafNext();
  ...
  prevCursor (cursor: Cursor): ?Cursor {
    const leaf = cursor.getLeaf();
    const prevIndex = cursor.getIndex() - 1;
    if (leaf.getChildByIndex(prevIndex)) {
      return new Cursor(leaf, prevIndex);
    } else {
      const prevLeaf = leaf.getLeafPrev();
      if (prevLeaf && prevLeaf.getChildByIndex(prevLeaf.getChildrenLength() - 1)) {
        return new Cursor(prevLeaf, prevLeaf.getChildrenLength() - 1);
      }
    }
    return null;
  }
  ...
  insertEntryPairRoot (
    ...
    const [{key: keyFirst}, leafFirstNew] = leafFirst.insertEntry(entryOpen, 0);
    const [{key: keyLast}, leafLastNew] = leafLast.insertEntry(entryClose, -1);
    return [
      new GapLink(leafFirstNew, keyFirst),
      new GapLink(leafLastNew, keyLast)
    ];
  }

  insertEntryPairChild (
    [entryOpen, entryClose]: [OrderEntry, OrderEntry]
    [parentLinkOpen, parentLinkClose, position]: [GapLink<Leaf>, GapLink<Leaf>, number]
  ) {
    const cursorParentOpen = this.findCursor(parentLinkOpen);
    const cursorParentClose = this.findCursor(parentLinkClose);
    if (!cursorParentOpen || !cursorParentClose) {
      throw new Error('Invalid parent links');
    }
    if (position > 0) {
      // we assume this is the child cursor
      // cursorNext is initialised as the nextCursor
      // but afterwards we are going to use nextSiblingCursors
      // if we start at position 0, then we need to insert at the left hand side
      // oh no, it's the right side of the cursorParentOpen

      const cursorTarget = this.nextCursor(cursorParentOpen);
      while (position > 0) {
        --position;
        const siblingCursors = this.nextSiblingCursors(cursorTarget);
        if (siblingCursors) {
          cursorTarget = siblingCursors[1];
        } else {
          cursorTarget = this.nextCursor(cursorTarget);
          break;
        }
      }
      ...
      // we need to be able to say that we will insert on the left hand side
      // because at position 0, we insert immediately

      // we need the cursorTarget
      // so we insert on the left hand side
      this.insertEntryPairSibling([entryOpen, entryClose], this.getLink(cursorTarget));

      // the insertion is always insertion onto the left or right or some link
      // the target cursor if open, is insertion on the left
      // if the target cursor is closed, it's insertion on the right


      // if the cursorNext is at the cursorParentClose
      // then we are finished

        while (cursorNext != )

        // we are going reduce the position one by one until it reaches 0
        // note that 0 means insert now
        // and 1 means insert after the first element
        // so which ever we iterate to we are always going to insert on the right hand side
        // get the children first
        // we need to use the cursor to the next element


        // as long as this next cursor is not a child


        this.nextSiblingCursors();
      ...
      const cursorPrev = this.prevCursor(cursorParentClose);
  ...
  insertEntryPairSibling (
    [entryOpen, entryClose]: [OrderEntry, OrderEntry],
    siblinglink: GapLink<Leaf>
  ): [GapLink<Leaf>, GapLink<Leaf>] {
    // insertion as sibling
  }

  // position notation should cover
  // insertion as child of NODE (position pointer)
  // or insertion as sibling of NODE (position pointer (left/right))
  // note that insertion as child of none, means insertion into root
  // inserstion as sibling of root, means you can have multi-roots
  // which means you actually have multiple roots
  // we can simplify this by making sure you always have a root
  // so you cannot insert as sibling of root
  // you can only insert as sibling if the sibling has the same parent!
  // what kind of notation are we dealing with?

  // for child:
  // openParentLink, closeParentLink, positionNumber (0.. or -1..)
  // for sibling
  // openSiblingLink -> means insertion onto the left
  // closeSiblingLink -> means insertion onto the right
...
// by not exporting the cursor, we can make sure that cursor construction always occurs according to an entry that exists
// but also this is problematic since you could findCursor() and get a cursor, then delete the underlying entry and node, and then it's gone, that cursor is now now relevant
export type { Cursor };
