...
class BOTree implements OrderIndexI<GapLink<Leaf>, Cursor> {
  ...
  insertEntryPairChild (
    ...
    const parentCursorOpen = this.findCursor(parentLinkOpen);
    const parentCursorClose = this.findCursor(parentLinkClose);
    if (!parentCursorOpen || !parentCursorClose) {
    ...
    if (cursorTarget === parentCursorClose || cursorTarget === parentCursorOpen) {
      const [
        {key: keyOpen},
        leafOpen,
        indexOpen
      ] = parentCursorOpen.getLeaf().insertEntry(
        entryOpen,
      ...
      const [
        {key: keyClose},
        leafClose
      ] = leafOpen.insertEntry(
        entryClose,
        indexOpen + 1
      );
      return [new GapLink(leafOpen, keyOpen), new GapLink(leafClose, keyClose)];
    }
    if (position >= 0) {
      while (position > 0) {
        --position;
