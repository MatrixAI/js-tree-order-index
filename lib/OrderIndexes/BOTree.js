...
class Leaf {
  ...
  caret (position: number, value: OrderEntry): GapKey {

    // note that careting may not be possible if we are full
    // so we need a function that checks before hand
    // something that says if we are to insert
    // then if we are not full, we caret
    // if we are full, we split

    position = boundIndex(position, this.children.count);


    // not so simple, we need to generate the gapkey
    // and we need to return the GapKey that we are using
    // as that will be used
