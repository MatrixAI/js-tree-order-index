...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  insertChild (
    ...
    // how can we be sure that openLink and closeLink is under the same node
    // actually we don't need to
    // all we are saying is that if we have a number
    // we just start iterating from linkOpen and linkClose
    // that's all
    // this is a CHILD from the POV of the BOTree
    // so it's fine
