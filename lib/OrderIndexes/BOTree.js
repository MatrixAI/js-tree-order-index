...
class Leaf {
  ...
  generateGapKey (index: number): ?GapKey {
  ...
  caret (
    ...
    let gapKey;
    gapKey = this.generateGapKey(position);
    if (gapKey == null) {
    ...
    if (needsRelabelling) {
      this.relabelGapKeys(updateLink);
  ...
  caretPair (
    ...
    let needsRelabelling = false;
    let gapKey1;
    gapKey1 = this.generateGapKey(position1);
    if (gapKey1 == null) {
      needsRelabelling = true;
      gapKey1 = 0;
    }
    ...
    let gapKey2;
    gapKey2 = this.generateGapKey(position2);
    if (gapKey2 == null) {
      needsRelabelling = true;
      gapKey2 = 0;
    }
  ...
  split (
    ...
    // what happens when we slice with an incorrect number
    // that is slice with 1.5
    ...
    // when we split
    // we set the splitted prevId to us
    // HOWEVER
    // if there was already something thinking that we are this
    // we need to update them to point to us
    // that's the idea
    // so actually this causes a big problem
    // it means every update
    // every split will propagate a change to every block to the right
    // no it doesn't propagate
    // only the existing next block
    // must have its reverse pointer point back to the new split
    // but that means the new block must be set in the table
    // we must clone it and... you know
    // we cannto do it here, cause we don't have access to the treeTableT

    leafRight.relabelGapKeys(updateLink);
...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  _updateLink = (
    nodeTableT: NodeTableTransaction,
    id: NodeId,
    openLink: ?LinkOpen,
    closeLink: ?LinkClose
  ) => {
    ...
    if (!updatedNode) {
      throw new Error('Incorrect id when updating the node table!');
    }
    return;
  }
  ...
  constructor (
    ...
    if (!config.treeTable) {
      ...
      // we pass in the concept of the _updateNode
      // everything else should be passing this on!
      const blockRoot = new Leaf(
        id,
        0,
        new ArrayFixedDense(config.blockSize),
        this._updateNode
      );
  ...
  _insertIntoLeaf ({
    ...
    console.log('Inside insertIntoLeaf');

    if (leaf.space > 1) {

      console.log('Inside no need to split!');
      ...
      console.log('Inside needing to SPLIT!');
      ...
      console.log('Splitting the leaf', leafLeft.id);

      [leafLeft, leafRight] = leafLeft.split(updateLink, counterT.allocate());

      console.log('Splitted into', leafLeft.id, ':', leafRight.id);

      if (leafRight.nextId != null) {

        console.log('Updating right-right leaf', leafRight.nextId);

        let leafRightRight = treeTableT.get(leafRight.nextId);
        if (!snapshot.has(leafRightRight)) {
          leafRightRight = leafRightRight.clone();
          snapshot.add(leafRightRight);
        }
        leafRightRight.prevId = leafRight.id;

        console.log(
          leafRightRight.prevId + ' <- ' + 'RightRight leaf ' + leafRightRight.id + ' -> ' + leafRightRight.nextId
        );

        treeTableT.set(leafRightRight.id, leafRightRight);
      }

      console.log(
        leafLeft.prevId + ' <- ' + 'Left leaf ' + leafLeft.id + ' -> ' + leafLeft.nextId
      );
      console.log(
        leafRight.prevId + ' <- ' + 'Right leaf ' + leafRight.id + ' -> ' + leafRight.nextId
      );

      // this is correct
      console.log('leafLeft children', leafLeft.children._array);
      console.log('leafRight children', leafRight.children._array);

      // if the leaf we split was blockRight
      // then the new blockRight is the leafRight
      if (leaf === blockRight) {
        console.log('Updating blockRight to be leafRight');
      ...
      if (indexSplit < indexCenter) {
        console.log('leafLeft is leafInserted');
        leafInserted = leafLeft;
        index = indexSplit;
      } else {
        console.log('leafRight is leafInserted');
        leafInserted = leafRight;
        index = indexSplit - indexCenter;
      }

      console.log('inserted position is', index);

      const [gapKey] = leafInserted.children.get(index);

      console.log('inserted gapKey is', gapKey);

      // this adds it to the snapshot
      // cause we know either leafLeft is a clone or that it is already in the snapshot
      // leafRight was newly created, so we add it to the snapshot
      ...
      // add in the new leafRight
      treeTableT.set(leafRight.id, leafRight);

      // it should be 4 things, the original 3 and the new split
      // this is correct
      console.log('treeTable at this point', treeTableT);

      if (!leaf.parentId) {

        // this is not relevant here, as all our leafs have parents here
        console.log('leaf does not have a parent');
        ...
        console.log('proceeding to node insertion');
  ...
  _insertIntoLeafPair({
    ...
    // so this checks if the space is 2
    if (leaf.space > 2) {
      ...
      // the original leafRight if it exists must be updated
      // to point its `prevId` to the new leafRight
      if (leafRight.nextId != null) {
        let leafRightOrig = treeTableT.get(leafRight.nextId);
        if (!snapshot.has(leafRightOrig)) {
          leafRightOrig = leafRightOrig.clone();
          snapshot.add(leafRightOrig);
        }
        leafRightOrig.prevId = leafRight.id;
        treeTableT.set(leafRightOrig.id, leafRightOrig);
      }
  ...
  _insertIntoNode<Result: { blockRoot: Tree }> (
    ...
    console.log('Inside insertIntoNode');
    console.log('Node', node.id);
    console.log('Node children', node.children._array);
    console.log('From existingId', existingId, 'inserting newId', newId);


    if (node.space > 1) {

      console.log('Node does not need to split!');
      ...
      console.log('Node needs to split!');
  ...
  insertRoot (
    ...
    const [[
      ...
      if (
        ...
        // these 2 functions are not working
        // when we have to do a tandem split
        // from both sides
        // it's creating something horribly wrong
        // firstly it is inserting fine when we don't have to split


        // the basic idea with this function
        // is to only insert 1 THING
        // which may cause the system to split
        // because the idea is that if it only 1 space left
        // it will split
        // hence then it will always have 1 space
