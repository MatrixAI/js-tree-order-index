...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  // everything wraps into this function
  // this ensures success by making sure that every time we insert
  // we are going to try splitting
  // this means leaf insertion and node insertion is recursive
  // so we try to insert entry which leads to insert into node

  // when we are splitting children and inserting into the parent
  // the only thing we can do is caret into the next part
  // next to the child being split
  // that's it
  ...
  _insertIntoNode (
    ...
    snapshot: SnapShot,
    node: Node,
    existingChild: TreeId,
    newChild: TreeId,
    results: [Leaf, GapKey, number]
  ) {

    // this is only used for splitting


    // tail recursive up the root
    // we need to carry the results up
    return results;
  }
  ...
  _insertIntoLeaf (
    ...
    position: number,
    entry: OrderEntry
  ): [Leaf, Gapkey, number, ?Node] {
    // if there are 2 spaces at least
    // then we can just insert with no problems
    // if there are not
    // this means we need to split
    // this ensures that we are always balanced
    if (leaf.space > 1) {
      let leafNew;
      ...
      return [leafNew, gapKey, index, null];
    } else {

      // there's only 1 space left
      // we can caret in
      // but we must split

      let leafLeft, leafRight;
      if (!snapshot.has(leaf)) {
        [leafLeft, leafRight] = leaf.splitI(counterT.allocate());
        snapshot.add(leafLeft);
      } else {
        [leafLeft, leafRight] = leaf.split(counterT.allocate());
      }
      ...
      let index, gapKey, leafCaretted;
      position = boundIndex(position, leaf.children.length - 1);
      if (position < (leaf.children.length / 2)) {
        [index, gapKey] = leafLeft.caret(position, entry);
        leafCaretted = leafLeft;
      } else {
        [index, gapKey] = leafRight.caret(position - (leaf.children.length / 2), entry);
        leafCaretted = leafRight;
      }
      ...
      if (!leaf.parentId) {
        ...
        // this level is what?
        // oh yea... a level creation here needs to do what exactly?
        // not sure
        // this the level created from insertion
        ...
        leafLeft.parentId = parentNode.id;
        leafRight.parentId = parentNode.id;
        treeTableT.set(parentNode.id, parentNode);

        // wait...
        // while this setups the tree
        // we don't have information on who's left
        // who's right
        // and who's root
        // all this may have changed
        // leftId can be taken care of outside
        // but the new root id must be returned as well
        // since there may be a new root id created

        // we are saying that we know that he parentNode is the new root
        return [leafCaretted, gapKey, index, parentNode];
      } else {
        const node = treeTableT.get(leaf.parentId);
        return this._insertIntoNode(
          ...
          leafLeft.id,
          leftRight.id,
          [leafCaretted, gapKey, index, null]
        );
      }
    }
    ...
    // so this works within a transactional context
    // and there we go!

    // we should allow multiple inerstions to generalise the insertion length
    // but note that we are dealing a block size
    // we are saying that we can insert at least half
    // of the block size
    // in one go
    // if we are trying to insert more than half
    // we need to do it twice
    // but splitting it twices
    // or maybe we split is 3 times to insert
    // or we always allow inserting 2 things
    // or inserting UP to the block size
    // in which case we are saying that if we are inserting more than the block size we need to split multiple times
    // so we can do it with a list of entries
