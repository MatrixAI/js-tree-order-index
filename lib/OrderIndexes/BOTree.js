...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  _updateLink (
    ...
    // we just do and reassign globally?
    // or we can do it in a transactional context
    // so that way
    // OR... we always
  ...
  insertRoot (
    ...
    const treeTable = this._treeTable.withMutations(treeTableT => {
      let left, right;
      // if left === right, then left === root AND right === root
      // there's only 1 block
      if (this._leftId === this._rightId === this._rootId) {
        const block = treeTableT.get(this._leftId);
        if (!(block instanceof Leaf)) {
          throw new Error();
        }
        if (block.space >= 2) {
          const rootEntry = nodeTableT.getNode(block.children.get(0));
          let levelRootNew;
          if (rootEntry) {
            levelRootNew = nodeTableT.getNode(rootEntry.id).level - 1;
          } else {
            // the only way this occurs is that you are the root
            // the root is the leaf
            // and the only level that exists is on the current leaf
            // in that case we just make sure to set the leaf
            // we take the level of teh current block
            // and we just minus it
            levelRootNew = -this.level;
          }

          /* const blockRootNew = treeTableT.get(this._rootId).clone();
           * blockRootNew.level++;
           * treeTableT.set(blockRootNew.id, blockRootNew);*/
          ...
          [, gapKeyOpen] = blockNew.caret(0, entryOpen);
          [, gapKeyClose] = blockNew.caret(-1, entryClose);
          treeTableT.set(blockNew.id, blockNew);

          // if blockRootNew and blockNew
          // are the same block
