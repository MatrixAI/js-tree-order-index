...
class Leaf {
  ...
  // the idea is that it is part of the ndoe context
  relabelGapKeys (
    ...
    // in order to update
    // we need to pass the nodeTableT
    //
  ...
  caret (
    ...
    if (needsRelabelling) {
      this.relabelGapKeys(updateLink, [position]);
  ...
  caretPair (
    ...
    if (needsRelabelling) {
      this.relabelGapKeys(updateLink, [position1, position2]);
...
class BOTree implements OrderIndexI<LinkOpen, LinkClose, CursorEntry> {
  ...
  _insertIntoLeafPair({
    ...
    console.log('Within Insert Into Leaf Pair');
    // so this checks if the space is 2
    if (leaf.space > 2) {

      console.log('SPACE greater than 2');
  ...
  insertRoot (
    ...
    const [[
      ...
      if (
        ...
        const updateLink = (id, openLink, closeLink) => {
          if (!openLink && !closeLink) return;
          const updatedNode = nodeTableT.updateNode(
            id,
            {...openLink, ...closeLink}
        ...
        console.log('Within insertRoot onto single block!');

        const block = treeTableT.get(this._rootId);
        let levelRootDelta = 0;
        if (block.children.count) {

          console.log('count of block', block.children.count);

          const rootOpenEntry = block.children.get(0);

          console.log('root entry', rootOpenEntry);
