...
class Leaf {
  ...
  leafPrev: ?Leaf;
  length: number;
  children: Array<[number, OrderEntry]>;
  constructor () {

    // weaker constraint since we need to mutate
    // the length for the true array
    Object.seal(this);
  }
}
...
// we make sure objects we return are immutable
// we can make sure that our object are sealed for Node and Leaf that do require changes
// automatically
function mkCursor (leaf, index) {
  return Object.freeze({leaf: leaf, index: index});
}

// we can makesure that GapLink does that too!

// once constructed they should be frozen so they cannot be mutated
// but also Leaf and stuff should be mutated as well
// but also Object.freeze()
// when returning the cursor
