...
class Leaf {
  ...
  split (
    ...
    if (index === undefined) {
      index = Math.floor(this.children.count / 2);
    }
...
class Node {
  ...
  caret (position: number, value: TreeId): number {
    console.log('BEFORE CARET NODE', this.children._count);
    if (this.space < 1) {
      throw new Error('Not enough space to caret in 1 entry');
    }
    position = boundIndex(position, this.children.length - 1);
    this.children.caretRight(position, value);
    console.log('AFTER CARET NODE', this.children._count);
    return position;
  }

  split (idNew: TreeId, index?: number): [Node, Node] {

    console.log('BEFORE SPLIT NODE', this.children._count);
    ...
    console.log('AFTER SPLIT NODE', nodeLeft.children._count, nodeRight.children._count);
