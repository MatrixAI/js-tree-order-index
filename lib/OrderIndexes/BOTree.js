...
class Leaf {
  id: TreeId;
  children: ArrayFixedDense<[GapKey, OrderEntry]>;
  levelDelta: number;
  parentId: ?TreeId;
  prevId: ?TreeId;
  nextId: ?TreeId;
  ...
  // insertion into the leaf involves splicing and gapkey consideration
  insertion () {
...
class Node {
  ...
  insertion (inserted: TreeId, position: number) {

    // we only need to do splicing here
    // what are we inserting?
    // we are creating a new object here and giving it back or what?
    // we must bound the position
    // the last index in this case can be the count instead of the very last thing

    position = boundIndex(position, this.children.count);

    // we are talking about arrayfixeddense
    // for a fixed density, this requires pushing the elements
    // otherwise deletion actually needs to correspoind to insertion
    // one cannot delete

    // damn? so how do we caret in something new?
