...
class Leaf<entry> {
  ...
  id: TreeId;
  levelDelta: number;
  children: ArrayFixedDense<[GapKey, entry], true>;
  parentId: ?TreeId;
  prevId: ?TreeId;
  nextId: ?TreeId;
  ...
  caret (
    ...
    this.children.caretRight(position, [gapKey, entry_]);
    if (needsRelabelling) {
      this._relabelGapKeys(updateLink);
    ...
    console.log('caretted', gapKey, position);
  ...
  caretPair (
    ...
    // these are wrong and need to be fixed to the count instead
