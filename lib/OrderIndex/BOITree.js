...
class BOITree<data: *> implements OrderIndexI<LinkOpen, LinkClose, data> {
  ...
  insertRoot (
    ...
    return [nodeInserted, boiTree];
  }

  insertChild (
    parentNode: Node<LinkOpen, LinkClose, data>,
    position: number,
    data_: data
  ): [
    Node<LinkOpen, LinkClose, data>,
    ...
    // we are going to insert within the parent node
    // we use the parent node's link open and link close to do this
    // and that's how we use the position number
    // ah yes
    ...
    const [[
      ...
      (
        ...
        // we'll be using this for our insertions afterwards
        // we are only inserting one at a time
        ...
        // so we need to locate the parent node
        // we locate the link open and link close
        // this will point to leaf entries
        // and this will allow us to know which one we want to do
        // the position number tells us what direction we want to acquire
        // wait we already have the parent node
        // we don't need to use ids for that

        let cursor;
        if (position >= 0) {

          // if position is positive
          // we always try to insert next to the current cursor to the right
          // so the cursor will be on the left side
          // wait we are insertin into a leaf
          // we actually have to convert the cursor to the correct leaf position
          // it makes sense cause _insertIntoLeaf will take the leaf, position, entry
          // so we know the position we want to insert at!

          // we need to go down
          // and if we are down but there is no more
          // then we know we are just inserting
          cursor = this._getCursorOpen(parentNode);
          const result = cursor.down();

          // we go down
          // fi we are done, we know where we want to insert


          if (result.done) {

            // they could be the same if we are getting them
            // so we should check before hand

            // wait so we always have enough to insert 2
            // that's the idea
            // but how can we be sure?


            // so do we know what position we want?
            ...
            // position 0 means we are inserting here already
            // so that means we are already here
            // position 1 means our cursor needs to be after it
          ...
          // we want to jump into the child
          // but what if there is no child
          // well we would want to iterate into child
          // that's the idea
        } else {
  ...
  _insertIntoLeaf ({
    ...
    if (leaf.space > 2) {
      ...
      // oh are we even splitting correctly now?
      // you want to split at the count
      // not at the length
      ...
      console.log('leafLeft', leafLeft.children);

      const [, indexSplit] = leafLeft.caret(updateLink, position, entry);


      // ok so if this was 2 instead would it work?
      console.log('indexSplit', indexSplit); // why is this 3, shouldn't this be 2
      console.log('leafLeft', leafLeft.children);

      [leafLeft, leafRight] = leafLeft.split(updateLink, counterT.allocate());

      // i think the indexSplit is wrong
      console.log('leafLeft', leafLeft.children);
      console.log('leafRight', leafRight.children);
      ...
      // so here we are saying that there maybe 2 spaces or 1 spaces
      // left
      // actually in this case we can ensure that there will always be 2 spaces
      // not just 1 space left
      // so if there are always 2 spaces
      // then what does this mean?

      // indexSplit tells us its 2
      // 2 was the position where the thing was inserted
      // the split occurred at 1 inclusive
      // which means leafLeft has 0 and 1
      // while leafRight has 0 (which is actually 2)

      let leafInserted, index;
      if (indexSplit <= indexCenter) {
        console.log('indexSplit is less than indexCenter');
        leafInserted = leafLeft;
        index = indexSplit;
      } else {
        console.log('indexSplit greater than indexCenter');
        console.log(indexSplit, indexCenter);
        leafInserted = leafRight;
        index = indexSplit - indexCenter - 1;
      }

      // the index is still wrong here
      // the index should be 0
      // because we split in the middle of the count
      // and so the new thing is on the new leaf
      // and this should be the index
      console.log('index', index);
      // this index ends up being wrong?

      console.log('LEAF INSERTED', leafInserted.children);
