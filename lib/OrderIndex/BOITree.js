...
import type { BlockTree, BlockTreeId } from './BlockTree.js';
import type {
  GapLink,
  TableLinkOpen,
  TableLinkClose,
...
import { BlockLeaf, BlockNode } from './BlockTree.js';
import Cursor from './BOITreeCursor.js';
import { interpolationSearch, nestContexts } from '../utilities.js';

type GapKey = number;
type SnapShot = WeakSet<BlockTree<OrderEntry>>;
type BlockTreeTable = MapI<BlockTreeId, BlockTree<OrderEntry>>;
...
class BOITree<data: *> implements OrderIndexI<TableLinkOpen, TableLinkClose, data> {
  ...
  _treeTable: BlockTreeTable;
  _counter: CounterImmutable;
  _rootId: BlockTreeId;
  _leftId: BlockTreeId;
  _rightId: BlockTreeId;
  ...
  constructor (
    ...
    if (!config.treeTable) {
      ...
      const blockRoot = new BlockLeaf(id, 0, new ArrayFixedDense(config.blockSize));
  ...
  insertParent (
    ...
    const [[
      ...
      (
        ...
        // we need to split the tree first
        // to get a subtree
        // should we use a function to do that?
        // splitTree(leaf, position1, leaf2, position2)
        // range notation
        // to split the tree
  ...
  _insertIntoLeaf ({
    ...
    treeTableT: BlockTreeTable,
    counterT: CounterTransaction,
    snapshot: SnapShot,
    updateNodeLink: UpdateNodeLink,
    blockRoot: BlockTree<OrderEntry>,
    blockLeft: BlockLeaf<OrderEntry>,
    blockRight: BlockLeaf<OrderEntry>,
    leaf: BlockLeaf<OrderEntry>,
    position: number,
    entry: OrderEntry
  }): {
    blockRoot: BlockTree<OrderEntry>,
    ...
    if (leaf.space > 2) {
      ...
      if (leafRight.nextId != null) {
        ...
        if (!snapshot.has(leafRightRight)) {
          ...
          if (blockRight === leafRightRight) {
            // $FlowFixMe: leafRightRightNew is a BlockLeaf
      ...
      if (!leaf.parentLink) {
        const children = BlockNode.prepChildren(
          [leafLeft.id, leafRight.id],
          leaf.children.length
        );
        const blockRoot = new BlockNode(counterT.allocate(), 0, children);
        leafLeft.parentLink = {
          link: blockRoot.id,
  ...
  _insertIntoLeafPair({
    ...
    blockLeft: BlockLeaf<OrderEntry>,
    blockRight: BlockLeaf<OrderEntry>,
    leaf1: BlockLeaf<OrderEntry>,
    gapKey1: GapKey,
    index1: number,
    leaf2: BlockLeaf<OrderEntry>,
    gapKey2: GapKey,
    index2: number
  } {
    ...
    if (leaf.space > 3) {
      ...
      if (leafRight.nextId != null) {
        const leafRightRight = treeTableT.get(leafRight.nextId);
        if (leafRightRight instanceof BlockNode) throw new Error('leafRightRight must be a BlockLeaf');
        let leafRightRightNew;
        if (!snapshot.has(leafRightRight)) {
          leafRightRightNew = leafRightRight.clone();
          snapshot.add(leafRightRightNew);
  ...
  _insertIntoNode<Result: { blockRoot: BlockTree<OrderEntry> }> (
    {
      treeTableT,
      counterT,
      snapshot,
  ...
  // BlockTree exposes block.split
  // we need to do this recursively to retain a whole new tree
  // this is more extract tree range
  // and extracts a subtree
  // here, our subtree must be rooted
  // so actually all we need is a node
  // nothing else

  // we need to split the tree rooted at a node
  // we would would not bother with the cursors
  // we just take the node
  // and we acquire it as a separate tree
  // should we do it against a node?
  // or do it against a id?
  // note that node is an actual thing from the node table
  // which requires calling `const childNode = ...`
  // so this avoids double calling!


  // remember that if we are insertingParent multiple times
  // we are carrying on the snapshot
  ...
  _splitTree ({
    ...
    // so we are splitting a range?
    // how can we do this
    // with cursors then?

    // copy the node if doesn't already exist
    // update the tree table

    if (cursorOpen.leaf === cursorClose.leaf) {

      const leaf = cursorOpen.leaf;
      if (!snapshot.has(leaf)) {
        leaf = leaf.clone();
        snapshot.add(leaf);
        treeTableT.set(leaf.id, leaf);
      }

      const leafExtracted = lca.splice(
        updateNodeLink,
        counterT.allocate(),
        cursorOpen.index,
        cursorClose.index - cursorOpen.index + 1
      );

      snapshot.add(leafExtracted);
      treeTableT.set(leafExtracted.id, leafExtracted);


      // because this is happening within the current snapshot
      // we are just putting it there in the curren snapshot
      // so we can just split again if we need to

      treeTableT.set(lcaExtracted)
      snapshot.add(lcaExtracted);


      // the splitTree is just its own tree
      // do we put it in the TreeTable?
      // or what?
      // we may yet merge it
      // so we should add it in
      return splitTree;

    } else {
      // these are the inbetween leafs
      const leafs = [];
      for (
        let l = this._treeTableT.get(cursorOpen.leaf.nextId);
        l !== cursorClose.leaf;
        l = this._treeTableT.get(l.nextId)
      ) {
        leafs.push(l);
      }
    }
    ...
    // returns the split tree and the original tree
  }
