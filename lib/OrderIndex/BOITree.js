...
class BOITree<data: *> implements OrderIndexI<TableLinkOpen, TableLinkClose, data> {
  ...
  _splitTree ({
    ...
    const cursorOpen = this._getCursorOpen(node);
    const cursorClose = this._getCursorClose(node);
    if (cursorOpen.leaf === cursorClose.leaf) {

      // this splice operation leafLeft (which is still in the tree)
      // and leafRight which is now a new leaf
      // but not connected to the tree

      const lca = cursorOpen.leaf;
      [leafLeft, leafRight] = lca.splice(
        updateNodeLink,
        counterT.allocate(),
        cursorOpen.index,
        cursorClose.index + 1
      ...
      // then the split tree should be simple
      // the LCA is the current leaf
      // so we just perform a split on the current tree
