...
// cursor.next()
// cursor.prev()
// then we ask the tree to get us the link itself
// cursor.getTree()
...
class Cursor<data> implements OrderCursorI {
  ...
  // consider this a friend class
  // so we are accessing the private parameters
  ...
  toLink (): LinkOpen|LinkClose {
    ...
    // maybe we need to differentiate it even more easily
    // by using classes
    // or we can just use a tag
...
class BOITree<data: *> implements OrderIndexI<LinkOpen, LinkClose, data> {
  ...
  _getCursor (link: LinkOpen|LinkClose): OrderCursorI {
    let leafId;
    let gapKey;
    if (link.hasOwnProperty('leafOpenId')) {
      leafId = link.leafOpenId;
      gapKey = link.gapKeyOpen;
    } else if (link.hasOwnProperty('leafCloseId')) {
      leafId = link.leafCloseId;
      gapKey = link.gapKeyClose;
    }
    const leaf = this._treeTable.get(leafId);
