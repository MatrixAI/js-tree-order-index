...
class BOITree<data: *> implements OrderIndexI<Cursor, LinkOpen, LinkClose, data> {
  ...
  lastCursor (): ?Cursor {
    ...
    if (leafRight.children.get(leafRight.children.count - 1)) {
      ...
      return {
        treeTable: this._treeTable,
        leaf: leafRight,
        index: leafRight.children.count - 1
      };
    } else {
      return null;
    }
  ...
  nextCursor (cursor: Cursor): ?Cursor {
    if ((cursor.leaf.children.count - 1) > cursor.index) {
      return {
        ...cursor,
        index: cursor.index + 1
      };
    } else if (cursor.leaf.nextId != null) {
      const leafNext = cursor.treeTable.get(cursor.leaf.nextId);
      if (leafNext.children.get(0)) {
        // $FlowFixMe: leafNext is a leaf
        return {
          ...cursor,
          leaf: leafNext,
          index: 0
  ...
  prevCursor (cursor: Cursor): ?Cursor {
    ...
    } else if (cursor.leaf.prevId != null) {
      const leafPrev = cursor.treeTable.get(cursor.leaf.prevId);
      if (leafPrev.children.get(leafPrev.children.count - 1)) {
        // $FlowFixMe: leafPrev is a leaf
        return {
          ...cursor,
          leaf: leafPrev,
          index: leafPrev.children.count - 1
        };
      }
    }
    return null;
