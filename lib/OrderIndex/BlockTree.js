...
class BlockLeaf<entry> {
  ...
  // extracts a BlockLeaf out of it
  // we are extracting a blockleaf out of a blockleaf
  // that's how we split the tree
  // by default this extracts the entire BlockLeaf
  // out of it
  // which means you are left with an empty blockleaf
  // are we meant to use the same levelDelta?

  // we are slicing the block
  // but we are modifying the current tree as well
  // or we are extracting
  ...
  extract (
    ...
    const childrenExtracted = this.children.splice(start, deleteCount);
    // the current children may have shifted within the block
    this._relabelGapKeys(updateLink);
    const leafExtracted = new BlockLeaf(
      idNew,
      this.levelDelta,
      childrenExtracted,
      undefined,
...
class BlockNode {
  ...
  extract (
    ...
    // the current children may have shifted within the block
    this._relabelGapKeys(updateLink);
    const nodeExtracted = new BlockNode(
      idNew,
      this.levelDelta,
      childrenExtracted,
      undefined,
