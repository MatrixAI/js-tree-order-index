...
// oh wait, how do I genericise the type?
// it's Node<linkOpen, linkClose, data>
// but there's no way to say that what those are together?
// these need to be private functions _insertNode
// and both must then call this._insertNode
// but this now always refers back to the orignal object
...
> {
  ...
  _deleteNode (
    ...
    // ds.pull doesn't work on conn
    // only the db itself
    // so now what?
    // what about retract entity
    // alternatively we can perform a query, instead of the pull API
    // which should be able to use the connection API
    // this still works, which is good
    // we don't are about the report in this case?
    // wait this is a single transaction
    // and I want to apply multiple things
    // how do i apply in the middle and still request it
    // without having to generate a whole new table each time?

    // instead of prequerying it
    // we can use the transaction report to get whether anything actually changed
    // it will return the node that was retracted if necessary
    // report.tx_data === [entities?]
    // damn so actually connections are just dbs anyway
    // i mean they just allow you to perform a transaction
    // hmmm, need to think about this

    ds.transact(conn, [[':db.fn/retractEntity', 1]]);
  ...
  updateNode (
    ...
    if (Object.keys(nodeUpdated).length) {
      ...
      if (Object.keys(nodeDiff).length) {
        ...
        nodeTable = new NodeDataScript({new: false, db: db, tagger: tagger});
      } else {
        // if nothing changes, there's nothing to do
        // except to return the node being "updated"
        nodeTable = this;
      }
  ...
  /* transaction (callback: (NodeTableTransaction) => any) {
   *   const changed = new Reference(false);
   *   const conn = ds.conn_from_db(this._db);
   *   const tagger = this._tagger.transaction((tt) => {
   ...
   *         id
   *       ),
   *       updateNode (id, nodePatch) => updateNode(
   *         conn,
   *         tt,
   *         changed,
   *         id,
   *         nodePatch
   *       ),
   *       searchNodes (key, value) => searchNodes(
   *         conn,
   *         tt,
   *         changed,
   *         key,
