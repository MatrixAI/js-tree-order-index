...
class OrderIndexedTree<data, cursor> {
  ...
  _nodeTable: NodeTableI<GapLinkOpen, GapLinkClose, data>;
  _orderIndex: OrderIndexI<GapLinkOpen | GapLinkClose, cursor>;

  constructor (
    nodeTable: NodeTableI<GapLinkOpen, GapLinkClose, data>,
    orderIndex: OrderIndexI<GapLinkOpen, GapLinkClose, cursor>,
  ) {
  ...
  insertNode (
    ...
    OrderIndexedTree<data, cursor>
  ] {
    let nodeInserted, nodeTable, orderIndex;
    nodeTable = this._nodeTable.transaction(nodeTableT => {
      if (!position) {
        let openlink, closeLink, level, fillId;
        [
          [openLink, closeLink, level, fillId],
          orderIndex
        ] = this._orderIndex.insertRoot(nodeTableT);
        ...
        [
          [openLink, closeLink, level, fillId],
          orderIndex
        ] = this._orderIndex.insertChild(
          nodeTableT,
          posNode, // node is a merge of linkOpen
          posNode, // node is a merge of linkClose
          posIndex
        );
        nodeInserted = nodeTableT.insertNode(
          level,
    ...
    return [
      nodeInserted,
      new OrderIndexedTree(nodeTable, orderIndex)
    ];
  }
