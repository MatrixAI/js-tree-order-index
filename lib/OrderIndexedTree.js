...
class OrderIndexedTree<data, link, cursor> {

  // we don't care what kind of cursor it is
  // the cursor any type should have been an encapsulated
  // type variable
  // a true existential type
  // but there's no way to create this
  // https://stackoverflow.com/q/48399024/582917
  _index: OrderIndexI<data, link, cursor>;
  // actually all 3 are any
  // cause we don't care what link is, and we are not generic across links
  // we just don't care at all
  // wait.. but we may care that certain functions must use
  // a special type called any
  // that is if something returns a cursor
  // then it must then be used
  // note that will cursors be returned back to the user?
  // if so, they must be aware of it
  // it cannot be just any then?
  // cause we may need to refer that cursor type itself

  constructor (index: OrderIndexI<link, data, *>) {
    this._index = index;
  }
